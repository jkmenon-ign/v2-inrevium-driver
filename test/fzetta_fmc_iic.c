...
/*
 * Initialize IIC IP and its corresponding drivers and configptr instances
 */
int fzetta_fmc_iic_init(u8 Dev_ID) {
   ...
   fzetta_fmc_Iic_ConfigPtr = XIic_LookupConfig(Dev_ID);
    ...
    Status = xiic_initialize(&fzetta_fmc_Iic, "i2c");
...
/***************************************
 * Fidus Zetta FMC Xbar Switch (DS10CP15A)
...
int fzetta_fmc_iic_xbar_register_write(u8 RegAddr, u8 RegData)
{
   ...
   static u8 WriteBuffer[2];
    ...
    printf("%s: fzetta_fmc_Iic.BaseAddress : %lx \n", __func__, fzetta_fmc_Iic.BaseAddress);
