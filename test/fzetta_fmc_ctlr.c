...
int fzetta_fmc_register_write(fzetta_dev_type *Dev, u8 *Channel, spi_slave_sel *Slave_Sel, u8 *RegAddr, u8 *RegData){
	...
	if (*Dev == IIC_Dev){
		printf("%s: Dev Type : IIC Dev \n", __func__);
		printf("%s: RegAddr: %x, RegData :%x \n", __func__, *RegAddr, *RegData);
		Status |= fzetta_fmc_iic_xbar_register_write(*RegAddr, *RegData);
		printf("%s: fzetta_fmc_iic_xbar_register_write DONE \n", __func__);
	}
	else if (*Dev == SPI_Dev){
		printf("%s: Dev Type : SPI Dev \n", __func__);
		printf("%s: Slavesel: %x channel: %d RegAddr: %x, RegData :%x \n", __func__,*Slave_Sel, *Channel, *RegAddr, *RegData);
...
int fzetta_fmc_dev_init(fzetta_fmc_reg *fzetta_fmc_reg_init){
    ...
    if (Status != XST_SUCCESS) {
		printf("%s: General Fidus FMC Initialization : FAILED  \n",__func__);
		return XST_FAILURE;
	}

    //Indicate init done by setting bit7 of GPIO out reg
    //fzetta_fmc_init_done();
    printf("General Fidus FMC Initialization Done\n\r");
	return XST_SUCCESS;
...
int fzetta_fmc_dev_errata_init(){
	...
	for(ch=0; ch<4; ch++){
		...
		switch (dev_id) {
			...
			default:
				printf("Channel %d No Errata for Reclocker Dev ID (0x%02x)\r\n",ch, dev_id);
		}

		//Actual Register Initiliazation Routine
	    if (Status == XST_SUCCESS) {
			 printf("Channel %d Reclocker Errata Init Done (Dev ID 0x%02x)\n\r", ch, dev_id);
		}
		...
		switch (dev_id) {
			...
			default:
				printf("Channel %d No Errata for Receiver Dev ID (0x%02x)\r\n", ch, dev_id);
		}
	    if (Status == XST_SUCCESS) {
		 	printf("Channel %d Receiver Errata Init Done (Dev ID 0x%02x)\n\r", ch, dev_id);
		}
    ...
    //Indicate init done by setting bit7 of GPIO out reg
    fzetta_fmc_init_done();
	printf("%s: FMC_INIT_DONE BY GPIO \n",__func__);
	return XST_SUCCESS;
...
int fzetta_fmc_init(){
   ...
   printf("fzetta_fmc_table_init done \n");
